...
/// Accumulates list presentation over a single render pass.
///
/// `Widget::render` takes `&self`, and a view's children cross an
/// `Arc<dyn Widget>` boundary before they report their allocations, so the
/// sink has to be shared and interior-mutable. It is created and consumed
/// inside one [`RetainedRenderer::render`] call and never escapes it.
type PresentationSink = Arc<Mutex<Presentation>>;
...
impl Widget for ViewWidget {
    fn render(&self, ctx: &RenderCtx, out: &mut Surface) {
        ...
        ViewRenderer::new(ctx, out, Arc::clone(&self.presentation), cursor).render_view(&self.view);
    }
}
    ...
    W: Write,
{
    ...
    /// Draw `view` and retain the list presentation it reported.
...
pub fn render_into_with_theme(view: &View, theme: Theme, out: &mut Surface) {
    let context = RenderCtx::new()
        ...
        .with_theme(theme);
    let sink: PresentationSink = Arc::new(Mutex::new(Presentation::default()));
    let cursor = cursor_placement(view).cloned();
    ViewRenderer::new(&context, out, sink, cursor).render_view(view);
}
