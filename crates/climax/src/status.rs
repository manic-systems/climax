...
impl Status {
    ...
    /// Frame rate hint sampled when the first live renderer starts.
    ///
    /// The coordinator keeps the maximum fps across live entries; changing fps
    /// after the runtime is live does not reconfigure the running renderer.
    #[must_use]
    ...
    /// A failure to draw the first frame is retained and reported by
    /// [`StatusRuntime::finish`] rather than discarded.
    #[must_use]
...
impl StatusRuntime {
    ...
    pub fn finish(mut self) -> Result<()> {
        let mut failure = std::mem::replace(&mut self.started, Ok(())).err();
        if let Some(id) = self.id.take() {
            collect_failure(&mut failure, self.coordinator.remove(id));
        }
...
impl StatusCoordinator {
    ...
    fn insert(&self, entry: StatusEntry, fps: u16) -> (u64, Result<()>) {
        let id = {
            let mut state = lock(&self.inner.state);
        ...
        self.ensure_runtime();
        (id, self.mark_dirty().map_err(output_error))
    }
    ...
    fn remove(&self, id: u64) -> Result<()> {
        ...
        match mode {
            ...
            // `Auto` is resolved to `Live`/`Silent` before coordinators are
            // built, so a raw `Auto` here means silent.
            StatusMode::Silent | StatusMode::Auto => {},
        }
        failure.map_or(Ok(()), Err)
    }
...
impl StatusStack {
    /// Children must not run while the entries lock is held, or a child
    /// touching the coordinator would nest locks.
...
impl Widget for StatusStack {
    ...
    fn tick_interest(&self) -> TickInterest {
        combine_tick_interest(
            self.widgets()
                .iter()
                .map(|widget| widget.as_ref().tick_interest()),
        )
    }
}
...
mod tests {
    ...
    fn status_stack_composes_multiple_animating_widgets() {
        let entries: Entries = Arc::new(Mutex::new(BTreeMap::from([
            (1, entry("first")),
            (2, entry("second")),
        ])));
