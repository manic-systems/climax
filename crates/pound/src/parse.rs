...
fn parse_cmd<'a>(
    ...
    apply_global_hits(&targets, &mut m, hits);
        ...
        "version"
            if spec.has_version_info()
                && !targets.iter().any(|target| {
                    target.arg.long == Some("version") || target.arg.aliases.contains(&"version")
                }) =>
        {
...
fn builtin_short(
    ...
    match ch {
        'h' if !targets.iter().any(|target| target.arg.short == Some('h')) => {
            Some(Error::Help(help::render(spec, globals)))
        },
        'V' if spec.has_version_info()
            && !targets.iter().any(|target| target.arg.short == Some('V')) =>
        {
            Some(Error::Version(help::version_line(spec)))
        },
        _ => None,
