...
that reaches `main` exits silently with status 130. `try_run` and
...
Typed prompts never produce that exit. A cancelled `select`, `multi_select`,
`search`, `text`, or `review` resolves to `PromptOutcome::Leave`, leaving the
handler to decide whether leaving is an error; only cancellation the handler
propagates — from a custom interaction driver or `with_terminal_application` —
reaches `main` and exits 130. Ctrl-C and Ctrl-D are offered to the widget
first: a widget or `ActionBinding` that claims either key keeps the session
running, and only an ignored Ctrl-C cancels or an ignored Ctrl-D ends input.
